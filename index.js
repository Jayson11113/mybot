...
// STABLE SESSION PATH: Prevents losing your login credentials on restarts
...
console.log(`📁 Target authentication profile path: ${AUTH_DATA_PATH}`);
...
const client = new Client({
    authStrategy: new LocalAuth({ 
        clientId: 'stable-bot-session', // Fixed ID to preserve authentication
...
function resolveBrowserExecutablePath() {
    // Let Puppeteer locate its own binaries automatically inside the Linux Docker container environment
    if (process.platform !== 'win32') {
        return undefined;
    }

    // Fallbacks for local Windows testing environment
    const candidates = [
